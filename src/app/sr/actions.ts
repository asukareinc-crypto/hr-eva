...
export async function updatePeriodMeta(id: string, formData: FormData) {
  const session = await requireSr();
  const period = await prisma.evaluationPeriod.findUnique({
    where: { id },
    include: { client: true, template: true },
  });
  if (!period || period.client.tenantId !== session.user.tenantId) {
    throw new ValidationError("対象の評価期間が見つかりません。");
  }
