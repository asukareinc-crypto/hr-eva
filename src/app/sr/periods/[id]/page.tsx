...
import { updatePeriodStatus, sendReminderForPeriod, updatePeriodMeta } from "@/app/sr/actions";
      ...
      <Card className="p-5">
        ...
        {period.status !== "DRAFT" && (
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded p-2 mb-3">
            この期間は既に <Badge tone={period.status === "OPEN" ? "green" : "yellow"}>{STATUS_LABEL[period.status]}</Badge> です。日付を変更すると、運用中の評価に影響する可能性があります。
          </p>
        )}
        <form
          action={async (fd) => {
            "use server";
            await updatePeriodMeta(id, fd);
          }}
